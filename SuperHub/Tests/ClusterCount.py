...
for dtime in ['[6, 18]']:
    for disc in ['0.001', '0.0025', '0.005']:
        for ncl in ['40', '60', '80', '100']:
            ...
            nfile = 'bcntwitter-septemberbin-sizes-kmeans-minloc20-minsize20-timed%s-nclust%s-nusr50#70000-r%s' % (dtime, ncl, disc)
            print 'Km %s %s %s %d' % (dtime, ncl, disc, count_clust(nfile, 20))
            nfile = 'bcntwitter-septemberbin-sizes-spectral-minloc20-minsize20-timed%s-nclust%s-nusr50#70000-r%s' % (dtime, ncl, disc)
            print 'SP %s %s %s %d' % (dtime, ncl, disc, count_clust(nfile, 20))


        for damp in ['0.5', '0.999']:
            nfile = 'bcntwitter-septemberbin-sizes-affinity-minloc20-minsize20-timed%s-damp%s-nusr50#70000-r%s' % (dtime, damp, disc)
