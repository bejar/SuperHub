...
def get_tweets(city):
    ...
    for item in r:
        if 'limit' in item:
            logging.warning('%d tweets missed', item['limit'].get('track'))
        elif 'disconnect' in item:
            logging.error('disconnecting because %s', item['disconnect'].get('reason'))
            wfile.close()
            return 0
            ...
            logging.info('TW: %d', i)
            if 'text' in item:
                logging.info('Text: %s', item['text'])
            logging.info('Time: %s', time.ctime(int(item['timestamp_ms'][0:-3])))
            logging.info('ID: %s', str(item['user']['id']))
            logging.info('--------------------')
